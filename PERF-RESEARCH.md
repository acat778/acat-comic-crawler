...
2026-07-02 已完成首轮性能优化，优先覆盖收益最高且风险可控的 P0/P1 项：

- 图片下载与上传改为章节内有限并发，默认并发数为 4，可通过 `CRAWLER_IMAGE_CONCURRENCY` 调整，最大限制 20。
- 移除每张图片上传后的固定 `sleep(500)`，默认不等待；如源站或后端需要限速，可通过 `IMAGE_DELAY_MS` 重新启用毫秒级间隔。
- JMComic API、图片下载和后端上传均复用带 keep-alive 的 HTTP Agent，减少重复 TCP/TLS 握手。
- 章节仍保持串行处理，避免并发创建章节、重试清理和任务状态更新之间产生竞态。

首轮优化没有启用章节级并行、元数据缓存和 scramble_id 缓存；这些项会改变任务调度、失败重试或跨章节共享状态，后续需要配合更完整的回归测试再推进。

## 后续优化方向（按收益排序）

| 优先级 | 优化项 | 预期提升 | 复杂度 |
|---|---|---|---|
| P0 | 去掉 `sleep(500)` | 300 张图省 150 秒 | 已完成 |
| P0 | 图片并发下载（10-20 并发） | 5-10x | 已完成 |
| P1 | 章节级并行处理 | 10 章省 ~80% 时间 | 中 |
| P1 | HTTP keep-alive（复用 axios 实例） | 减少 TLS 握手 | 已完成 |
...
历史代码中的 `await sleep(500)` 位于 `src/services/crawler.js` 的 API 路径和浏览器路径，每张图上传到后端后固定等待 500ms。这个延迟可能是为了：
1. 避免后端服务过载
2. 避免触发 JMComic 的速率限制

当前已改为 `IMAGE_DELAY_MS` 配置项，默认值为 `0`。后续如果继续提高并发，需要考虑：
